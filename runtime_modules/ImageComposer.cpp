// ImageComposer.cpp
// Windows DLL example: accepts two RGBA images of equal dimensions and blends them.
// Build with MSVC Developer Command Prompt:
// cl /LD /EHsc ImageComposer.cpp /Fe:ImageComposer.dll
...
MODULE_API int ComposeImagesRGBA(
    ...
    for (int index = 0; index < byteCount; ++index) {
        output[index] = static_cast<std::uint8_t>(inputA[index] * inverse + inputB[index] * alpha);
    }
    return 0;
}
