...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="studio"><ModuleLibrary onAdd={addModule} /><NodeCanvas nodes={nodes} links={links} selected={selected} onSelect={id => setSelected(id || null)} onAddLink={(from, to, toPort) => { setLinks(prev => [...prev, { from, to, toPort }]); addLog(`모듈 간 데이터 연결을 만들었습니다. · 입력 ${toPort + 1}`) }} onRemoveLink={(from, to, toPort) => { setLinks(prev => prev.filter(link => link.from !== from || link.to !== to || link.toPort !== toPort)); addLog('모듈 간 데이터 연결을 삭제했습니다.') }} onMoveNode={moveNode} onRemoveNode={removeNode} onUpdateSetting={(id, setting) => setNodes(prev => prev.map(node => node.id === id ? { ...node, setting } : node))} onUpdateRuntime={(id, field, value) => setNodes(prev => prev.map(node => node.id === id ? { ...node, [field]: value } : node))} /><OutputPanel running={running} logs={logs} /></main>
