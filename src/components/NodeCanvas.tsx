...
export type NodeData = { id: string; type: ModuleType; title: string; detail: string; icon: string; x: number; y: number; setting?: string }
...
export default function NodeCanvas({ nodes, links, selected, onSelect, onAddLink, onUpdateSetting }: {
  nodes: NodeData[]; links: Link[]; selected: string | null; onSelect: (id: string) => void; onAddLink: (from: string, to: string) => void; onUpdateSetting: (id: string, setting: string) => void
}) {
  ...
  return <section className="canvas-wrap" aria-label="노드 작업 공간">
    <div className="canvas-toolbar"><span className="breadcrumb">작업 공간 <i>/</i> 데이터 파이프라인</span><span className="connect-hint">{linkStart ? '대상 노드를 선택해 연결하세요' : '노드의 연결점을 눌러 흐름을 만드세요'}</span><div><button className="tool-button">↶</button><button className="tool-button">↷</button><button className="tool-button">⌗</button></div></div>
    <div className="node-canvas">
      ...
      {nodes.map(node => <article className={`flow-node ${node.type} ${selected === node.id ? 'selected' : ''}`} key={node.id} style={{ left: `${node.x}px`, top: `${node.y}px` }} onClick={() => onSelect(node.id)}>
        ...
        <div className="node-body">{node.type === 'input' ? <><span className="stream-dot" />내장 예시 데이터</> : node.type === 'process' ? <><span>신뢰도</span><b>{node.setting || '0.65'}</b></> : <><span>상태</span><b className="ready">준비됨</b></>}</div>
...
export const initialNodes: NodeData[] = [
  ...
  { id: 'detect', type: 'process', title: '사람 인식', detail: '객체 탐지 모델', icon: '◌', x: 320, y: 175, setting: '0.65' },
  { id: 'viewer', type: 'output', title: '이미지 뷰어', detail: '2D 결과 화면', icon: '▣', x: 585, y: 100 },
  { id: 'count', type: 'output', title: '값 표시', detail: '실시간 숫자 출력', icon: '№', x: 585, y: 292 },
]
