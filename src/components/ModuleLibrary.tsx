...
/**
 * 라이브러리에 기본으로 제공하는 예시 모듈입니다.
 * 네이티브 C++ 모듈은 서버가 runtime_module 폴더를 별도로 검색해 함께 표시합니다.
 */
...
/**
 * 좌측 모듈 라이브러리입니다.
 * 화면이 열리면 서버에 모듈 목록을 요청하고, 빌드된 .so/.dll/.dylib 파일을 자동 감지한 결과를 표시합니다.
 */
export default function ModuleLibrary({ onAdd }: { onAdd: (item: ModuleItem) => void }) {
  ...
  // 네이티브 모듈 검색은 화면 최초 진입 시 한 번 실행합니다.
  useEffect(() => {
    api('runtime-modules')
      .then(async response => {
        if (!response.ok) throw new Error('모듈 검색 실패')
        return response.json() as Promise<ModuleItem[]>
      })
      .then(setDetected)
      .catch(() => setScanError(true))
  }, [])

  // 입력이 없는 C++ 모듈은 시작 모듈로, 입력이 있는 모듈은 처리 모듈로 나누어 보여 줍니다.
  ...
  // 기본 처리 모듈 앞에 감지된 C++ 처리 모듈을 배치합니다.
  const displayGroups = [
  ...
  return <aside className="module-library" aria-label="모듈 선택 영역">
    <div className="panel-heading">
      <span>모듈 라이브러리</span>
      <button className="icon-button" aria-label="모듈 검색">⌕</button>
    </div>

    {displayGroups.map(group => <section className="module-group" key={group.title}>
      <h2>{group.title}</h2>
      {group.items.map(item => <button
        className={`module-card ${item.type}`}
        key={`${item.label}-${item.dllName || ''}`}
        onClick={() => onAdd(item)}
      >
      ...
      {(group.title === 'C++ 처리 모듈' || group.title === 'C++ 시작 모듈') && detected.length > 0 && (
        <p className="module-scan">● 실행 폴더에서 {detected.length}개 네이티브 모듈 감지</p>
      )}
      {group.title === 'C++ 처리 모듈' && scanError && (
        <p className="module-scan error">네이티브 모듈 목록을 확인할 수 없습니다.</p>
      )}
    </section>)}

    <p className="library-help">
      모듈 JSON의 inputTypes·outputTypes에 Image, Number, Text 또는 배열형(Image[])을 적어 데이터 연결 규칙을 설정할 수 있습니다.
    </p>
  </aside>
}
